...
For example, |xs = |{\tt "\txtl{(())()}\txbr{))}\txbl{()}"} is left-partially balanced because |txbr "((" ++ xs| is balanced --- we use coloring to help the reader parsing the string.
Note that {\tt "\txbr{(}()\txbr{(}"} |++| |xs| is also balanced.
For a counter example, the string |ys = | {\tt "()\txbr{(}()"} is not left-partially balanced --- due to the unmatched {\tt '\txbr{(}'} in the middle of |ys|, there is no |zs| such that |zs++ys| can be balanced.
...
For example, |xs| can be factored into two balanced substrings, {\tt "\txtl{(())()}"} and {\tt"\txbl{()}"}, separated by {\tt"\txbr{))}"}.
...
\end{code}
For example, |xs = | {\tt "\txtl{(())()}\txbr{))}\txbl{()}"} can be represented by a forest containing three trees:
\begin{spec}
ts = [txtl (Bin (Bin Nul Nul) (Bin Nul Nul)), {-"~~"-} Nul, {-"~~"-} txbl (Bin Nul Nul)] {-"~~,"-}
\end{spec}
where |txtl (Bin (Bin Nul Nul) (Bin Nul Nul))| prints to {\tt "\txtl{(())()}"},
|txbl (Bin Nul Nul)| prints to {\tt"\txbl{()}"}, and there is a |Nul| between
them due to the consecutive right parentheses {\tt"\txbr{))}"} in |xs|
(|Nul| itself prints to |""|).
...
The empty string can be represented by |[Nul]|, since |prF [Nul] = pr Nul = ""|.
...
If we expand the definitions, we have
%|inv prF| is given by
...
for clarity, we prefer to make the partiality explicit.
